...
class ImportExportController extends BaseApiController
{
    ...
    public function validateImport(Request $request): JsonResponse
    {
        ...
        $rules = match ($entity) {
            ...
            'bantuan-lumpsum', 'bantuan-perumahan' => [
                'salary_grade_code' => ['required', 'string', 'max:50'],
    ...
    public function export(Request $request, string $entity): StreamedResponse|JsonResponse
    {
        ...
        $headers = match ($entity) {
            ...
            'bantuan-komunikasi' => ['Basis Penentuan', 'Kode Kriteria', 'Nama Level / Posisi', 'Kategori Komunikasi', 'Nominal Bantuan', 'Periode', 'Ketentuan / Fasilitas', 'Status', 'Tanggal Dibuat'],
        ...
        $generator = function () use ($entity, $modelClass) {
            $query = match ($entity) {
                ...
                'bantuan-komunikasi' => BenefitPlafond::with(['grade', 'position'])->where('benefit_type', 'BANTUAN_KOMUNIKASI')->orderBy('id'),
            ...
            foreach ($query->cursor() as $item) {
                yield match ($entity) {
                    ...
                    'uang-perdin' => [
                        $item->masterJenjang?->code ?? '',
                        $item->masterJenjang?->name ?? '',
                        (string) ($item->masterJenjang?->level ?? ''),
                        (string) ($item->amount ?? 0),
    ...
    public function downloadTemplate(Request $request, string $entity): StreamedResponse|JsonResponse
    {
        $templates = [
            ...
            'uang-perdin' => [
                'headers' => ['master_jenjang_code', 'master_jenjang_name', 'amount', 'period_type', 'status'],
                'samples' => [
                    ['MGR', 'Manager', '1000000', 'HARIAN', 'ACTIVE'],
                    ['SPV', 'Supervisor', '500000', 'HARIAN', 'ACTIVE'],
                    ['GL', 'Group Leader', '350000', 'HARIAN', 'ACTIVE'],
                ],
            ],
            'bantuan-lumpsum' => [
                'headers' => ['salary_grade_code', 'category_name', 'amount', 'period_type', 'description', 'status'],
