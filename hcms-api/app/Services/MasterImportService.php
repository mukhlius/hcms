        ...
        return DB::transaction(function () use ($modelClass, $data, $moduleName) {
            ...
            foreach ($data as $item) {
                ...
                if ($modelClass === \App\Models\MasterJenjang::class) {
                    ...
                    $level = !empty($item['level']) ? (int)$item['level'] : null;
                ...
                if ($modelClass === \App\Models\BenefitPlafond::class) {
                    ...
                    if ($benefitType === 'KACAMATA') {
                        ...
                        $jenjangName = $item['master_jenjang'] ?? $item['jenjang'] ?? $item['jenjang_name'] ?? $item['name'] ?? null;
                        $code = !empty($item['master_jenjang_code'] ?? $item['kode_jenjang'] ?? $item['code'] ?? '') ? strtoupper(trim($item['master_jenjang_code'] ?? $item['kode_jenjang'] ?? $item['code'])) : null;

                        $masterJenjang = null;
                        if (!empty($item['master_jenjang_id'])) {
                            $masterJenjang = \App\Models\MasterJenjang::find($item['master_jenjang_id']);
                        }
                        if (!$masterJenjang && !empty($code)) {
                            $masterJenjang = \App\Models\MasterJenjang::where('code', $code)->first();
                        }
                        if (!$masterJenjang && !empty($jenjangName)) {
                            $masterJenjang = \App\Models\MasterJenjang::where('name', $jenjangName)->first();
                        }

                        if (!$masterJenjang) {
                            $display = $jenjangName ?? $code ?? '(kosong)';
                            throw new \Exception("Master Jenjang '{$display}' tidak ditemukan dalam katalog Master Jenjang. Pastikan menggunakan nama atau kode Master Jenjang yang valid (contoh: Manager, Supervisor, MGR, SPV).");
                        }
                    ...
                    } elseif ($benefitType === 'BANTUAN_KOMUNIKASI') {
                        $gradeId = null;
                        $positionId = null;

                        if (!empty($item['position_code'])) {
                            $positionId = \App\Models\Position::where('code', $item['position_code'])->value('id');
                        } elseif (!empty($item['position_name']) || !empty($item['position_title'])) {
                            $pName = $item['position_name'] ?? $item['position_title'];
                            $positionId = \App\Models\Position::where('title', $pName)->orWhere('code', $pName)->value('id');
                        } elseif (!empty($item['position_id'])) {
                            $positionId = $item['position_id'];
                        }

                        if (!$positionId) {
                            if (!empty($item['grade_code'])) {
                                $gradeId = \App\Models\Grade::where('code', $item['grade_code'])->value('id');
                            }
                            if (!$gradeId && !empty($item['grade_name'])) {
                                $gradeId = \App\Models\Grade::where('name', $item['grade_name'])->value('id');
                            }
                            if (!$gradeId && !empty($item['level'])) {
                                $gradeId = \App\Models\Grade::where('level', $item['level'])->value('id');
                            }
                            if (!$gradeId && !empty($item['grade_id'])) {
                                $gradeId = $item['grade_id'];
                            }
                            if (!$gradeId && !empty($item['salary_grade_code'])) {
                                $gradeId = \App\Models\Grade::where('code', $item['salary_grade_code'])->value('id');
                            }
                        }
                        ...
                        \App\Models\BenefitPlafond::updateOrCreate(
                            [
                                'benefit_type' => 'BANTUAN_KOMUNIKASI',
                                'grade_id' => $gradeId,
                    ...
                    } elseif (in_array($benefitType, ['BANTUAN_LUMPSUM', 'BANTUAN_PERUMAHAN'])) {
                        $salaryGradeId = null;
